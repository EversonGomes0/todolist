...
import br.com.everson.todolist.utils.Utils;
...
public class TaskController {
    ...
    public ResponseEntity create(@RequestBody TaskModel taskModel, HttpServletRequest request) {
        ...
        if (currentData.isAfter(taskModel.getStartAt()) || currentData.isAfter(taskModel.getEndAt())) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("A data de início ou término não pode ser anterior à data atual.");
        }
    ...
    public ResponseEntity update(@RequestBody TaskModel taskModel, @PathVariable UUID id, HttpServletRequest request) {
        ...
        var taskUptated = this.taskRepository.save(task);
